...
import { createTag, updateTag } from "@/app/(dashboard)/tags/actions";
import {
  Button,
  Card,
  CardContent,
...
export function TagForm(props: Props) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const res =
        props.mode === "create" ? await createTag(fd) : await updateTag(fd);
  ...
  return (
    <Card className="max-w-xl border-border-subtle">
      <CardHeader>
        <CardTitle>{props.mode === "create" ? "New tag" : "Edit tag"}</CardTitle>
      </CardHeader>
      <CardContent>
        <form className="space-y-4" onSubmit={onSubmit}>
          ...
          <Input
            ...
            placeholder="#3366CC"
            hint="6-digit hex with #. Leave empty for default styling."
            disabled={pending}
